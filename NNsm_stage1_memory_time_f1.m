cd('/home/xyw/matlabcode/nnsm_layer1/soil_memory');
clc
clear
...
%      end
%          SSM_Data=SSM_Data_us;
%      % when T=3, uncommont this
     
     sm_sum = nan(size(SSM_Data,1),size(SSM_Data,2));
     for i = 1:size(lat,1)
         for j = 1:size(lat,2)
              sm = reshape(SSM_Data(i,j,:),1,size(SSM_Data,3));
              sm(sm<0.04) = nan;
              sm(sm>0.7) = nan;
              ...
              if ~isempty(sm)
                 ...
                 psm(psm<0) = [];
                 dpsoil1=30; % unit mm
                 sm_sum(i,j) = sum(psm).*dpsoil1;% dpsoil1=surface soil moisture depth in NNsm is 100mm.
              end
         end
     end
    prec_sum = nansum(GPM_prec,3); 
     ...
     tauS = -(1/2)./log(pfrac);
    filename = ['/home/xyw/matlabcode/nnsm_layer1/soil_memory/NNsm_tauS_f1_',yrlist{yr},'.mat'];
    save(filename,'tauS','-v7.3');  
    filename = ['/home/xyw/matlabcode/nnsm_layer1/soil_memory/NNsm_pfrac_f1_',yrlist{yr},'.mat'];
...
% 
% for yr=1:length(yrlist)
%     load(['/home/xyw/matlabcode/nnsm_layer1/NNsm_tauS_f1_',yrlist{yr},'.mat']);
